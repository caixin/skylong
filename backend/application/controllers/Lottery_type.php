...
class Lottery_type extends MY_Controller
{
    ...
    public function index()
    {
        ...
        // get total.
        $total = $this->ettm_lottery_type_db->where($where)->count();
        ...
        // get main data.
        $result = $this->ettm_lottery_type_db->where($where)
            ->order($order)
    ...
    public function create($id=0)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $this->form_validation->set_rules($this->ettm_lottery_type_db->rules());

            if ($this->form_validation->run() == true) {
                $row['mode'] = array_sum($row['mode']);
                $this->ettm_lottery_type_db->insert($row);
            ...
            if ($id != 0) {
                $row = $this->ettm_lottery_type_db->row($id);
    ...
    public function edit($id)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $this->ettm_lottery_type_db->update($row);
    ...
    public function delete()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->ettm_lottery_type_db->delete($id);
