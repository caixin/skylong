...
class Lottery_record_change extends MY_Controller
{
    ...
    public function index()
    {
        ...
        //注單統計
        $bet_where['operator_id'] = $where['operator_id'];
        $bet_where['t.lottery_id'] = $where['lottery_id'];
        $bet_where['t.money_type'] = 0;
        $bet_where['t.qishu'] = array_column($result, 'qishu');
    ...
    public function edit($operator_id, $id)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $this->form_validation->set_rules($this->ettm_lottery_record_change_db->rules());

            if ($this->form_validation->run() == true) {
                $row['operator_id'] = $operator_id;
                $row['record_id'] = $id;

                $this->ettm_lottery_record_change_db->update($row);
