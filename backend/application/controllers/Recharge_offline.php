...
class Recharge_offline extends MY_Controller
{
    public $id = 0;
    ...
    public function index()
    {
        ...
        // get total.
        $join[] = [$this->table_ . 'bank t1', 't.bank_id = t1.id', 'left'];
        $total = $this->recharge_offline_db->where($where)->join($join)->count();
        ...
        // get main data.
        $result = $this->recharge_offline_db->where($where)
            ->select('t.*,t1.name bank_name,t1.image_url bank_logo')->join($join)->order($order)
    ...
    public function create($id=0)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $this->form_validation->set_rules($this->recharge_offline_db->rules());

            if ($this->form_validation->run() == true) {
                $row['user_group_ids'] = implode(',', $row['user_group_ids']);
                $this->recharge_offline_db->insert($row);
            ...
            if ($id != 0) {
                $row = $this->recharge_offline_db->row($id);
        ...
        $data['bank'] = $this->bank_db->getList();
    ...
    public function edit($id)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $this->recharge_offline_db->update($row);
    ...
    public function delete()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->recharge_offline_db->update([
                'id'        => $id,
                'is_delete' => 1
            ]);
    ...
    {
        $row = $this->recharge_offline_db->where([
            't.account' => $account,
            't.id <>'   => $this->id
        ])->result_one();
