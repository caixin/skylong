...
class Lottery_sort extends MY_Controller
{
    ...
    public function index()
    {
        ...
        // get total.
        $total = $this->ettm_lottery_sort_db->where($where)->count_change();
        ...
        // get main data.
        $result = $this->ettm_lottery_sort_db->where($where)
            ->order($order)
    ...
    public function edit($operator_id, $id)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $this->form_validation->set_rules($this->ettm_lottery_sort_db->rules());

            if ($this->form_validation->run() == true) {
                $row['operator_id'] = $operator_id;
                $row['lottery_id'] = $id;
                $this->ettm_lottery_sort_db->update($row);
            ...
            $row = $this->ettm_lottery_sort_db->row_change($id);
        }
