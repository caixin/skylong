...
class Activity extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $search_params = param_process($params, ['operator_id', 'asc']);
        ...
        $total = $this->activity_db->where($where)->join($join)->count();
    ...
    public function create()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $this->form_validation->set_rules($this->activity_db->rules());
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($row['operator_id'] == 0) {
                    ...
                    foreach ($operator as $key => $value) {
                        $row['operator_id'] = $key;
                        $id = $this->activity_db->insert($row);
                    }
                } else {
                    $id = $this->activity_db->insert($row);
                }
