			...
			<div class="col-xs-1" style="width:auto;">
				...
				<select name="interface" class="form-control">
					...
					<?php foreach (recharge_online_model::$interfaceList as $key => $val) : ?>
						<option value="<?= $key ?>" <?= isset($where['interface']) && $where['interface'] == $key ? 'selected' : '' ?>><?= $val ?></option>
					<?php endforeach; ?>
				</select>
			</div>
			<div class="col-xs-1" style="width:auto;">
				...
				<select name="payment" class="form-control">
					...
					<?php foreach (recharge_online_model::$paymentList as $key => $val) : ?>
						<option value="<?= $key ?>" <?= isset($where['payment']) && $where['payment'] == $key ? 'selected' : '' ?>><?= $val ?></option>
					<?php endforeach; ?>
				</select>
			</div>
			<div class="col-xs-1" style="width:auto;">
		...
		<table class="table table-hover">
			<thead>
				<tr>
					...
					<th><?=sort_title('interface', '接口', $this->cur_url, $order, $where) ?></th>
					<th><?=sort_title('payment', '付款类型', $this->cur_url, $order, $where) ?></th>
					<th width="150"><?=sort_title('pay_url', 'API网址', $this->cur_url, $order, $where) ?></th>
					<th><?=sort_title('m_num', '商户号', $this->cur_url, $order, $where) ?></th>
			...
			<tbody>
				<?php foreach ($result as $row) : ?>
					<tr>
						...
						<td><?=recharge_online_model::$interfaceList[$row['interface']] ?></td>
						<td><?=recharge_online_model::$paymentList[$row['payment']] ?></td>
						<td style="word-break: break-all;"><?=$row['pay_url'] ?></td>
						<td><?=$row['m_num'] ?></td>
						...
						<td>
							<?php if ($this->session->userdata('roleid') == 1 || in_array("{$this->router->class}/edit", $this->allow_url)) : ?>
								<button type="button" id="status1_<?= $row['id'] ?>" class="btn <?= $row['status'] == 1 ? 'btn-info' : 'btn-default' ?>" onclick="status_row(<?=$row['id']?>,1)"><?= recharge_online_model::$statusList[1] ?></button>
								<button type="button" id="status0_<?= $row['id'] ?>" class="btn <?= $row['status'] == 0 ? '' : 'btn-default' ?>" onclick="status_row(<?=$row['id']?>,0)"><?= recharge_online_model::$statusList[0] ?></button>
							<?php else : ?>
								<?= recharge_online_model::$statusList[$row['status']] ?>
							<?php endif; ?>
						</td>
						<td><?=$row['sort'] ?></td>
