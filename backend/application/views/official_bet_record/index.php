        ...
        <form method="post" action="">
            ...
            <div class="col-xs-1" style="width:auto;">
                ...
                <select name="status" class="form-control">
                    ...
                    <?php foreach (Ettm_official_bet_record_model::$statusList as $key => $val) : ?>
        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th><?= sort_title('name', '玩法', $this->cur_url, $order, $where) ?></th>
                    <th width="180"><?= sort_title('bet_values', '投注内容', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('odds', '赔率', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('return_money', '返利', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('p_value', '单注金额', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('bet_number', '注数', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('bet_multiple', '倍数', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('total_p_value', '投注总额', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('return_point', '返点金额', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('c_value', '派彩金额', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('bet_eff', '有效下注', $this->cur_url, $order, $where) ?></th>
            ...
            <tbody>
                <?php foreach ($result as $row) : ?>
                    <tr>
                        ...
                        <td><?= $row['qishu'] ?></td>
                        <td><?= $row['name'] ?></td>
                        <td style="word-break: break-all;"><?= $row['bet_values'] ?></td>
                        <td><?= $row['odds'] ?></td>
                        <td><?= $row['return_money'] ?></td>
                        <td><?= $row['p_value'] ?></td>
                        <td><?= $row['bet_number'] ?></td>
                        <td><?= $row['bet_multiple'] ?></td>
                        <td><?= $row['total_p_value'] ?></td>
                        <td><?= $row['return_point'] ?></td>
                        <td><?= $row['c_value'] ?></td>
                        <td><?= $row['bet_eff'] ?></td>
                        <td style="color:<?=base_model::getProfitColor($row['profit'])?>"><?= $row['profit'] ?></td>
                        <td><?= Ettm_official_bet_record_model::$statusList[$row['status']] ?></td>
