...
<div class="box">
    <div class="box-header">
        <form method="post" action="">
            ...
            <div class="col-xs-1" style="width:auto;">
                <label>活动名称</label>
                <input type="text" name="name" size="50" class="form-control" placeholder="请输入..." value="<?= isset($where['name']) ? $where['name'] : '' ?>">
            </div>
            <div class="col-xs-1" style="width:auto;">
                <label>活动类型</label>
        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th><?= sort_title('type', '活动类型', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('name', '活动名称', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('pic1', '首页轮播', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('pic2', '活动图片(模板1)', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('pic3', '活动图片(模板2)', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('status', '活动状态', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('sort', '排序', $this->cur_url, $order, $where) ?></th>
                    <th width="100"><?= sort_title('create_time', '发布时间', $this->cur_url, $order, $where) ?></th>
                    <th width="150">
            ...
            <tbody>
                <?php foreach ($result as $row) : ?>
                    <tr>
                        ...
                        <td><?= activity_model::$typeList[$row['type']] ?></td>
                        <td><?= $row['name'] ?></td>
                        <td><img src="<?= $row['pic1'] ?>" style="max-width:150px;max-height:100px;"></td>
                        <td><img src="<?= $row['pic2'] ?>" style="max-width:150px;max-height:100px;"></td>
                        <td><img src="<?= $row['pic3'] ?>" style="max-width:150px;max-height:100px;"></td>
                        <td>
                            <?php if ($this->session->userdata('roleid') == 1 || in_array("{$this->router->class}/edit", $this->allow_url)) : ?>
                                <button type="button" id="status1_<?= $row['id'] ?>" class="btn <?= $row['status'] == 1 ? 'btn-info' : 'btn-default' ?>" onclick="status_row(<?= $row['id'] ?>,1)"><?= activity_model::$statusList[1] ?></button>
                                <button type="button" id="status0_<?= $row['id'] ?>" class="btn <?= $row['status'] == 0 ? '' : 'btn-default' ?>" onclick="status_row(<?= $row['id'] ?>,0)"><?= activity_model::$statusList[0] ?></button>
                            <?php else : ?>
                                <?= activity_model::$statusList[$row['status']] ?>
                            <?php endif; ?>
                        </td>
                        <td><?= $row['sort'] ?></td>
