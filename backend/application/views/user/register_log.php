        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th><?= sort_title('agent_code', '代理邀请码', $this->cur_url, $order, $where) ?></th>
                    ...
                    <th width="450"><?= sort_title('create_ua', 'UA', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('create_domain', '注册网域', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('create_ip', '注册IP', $this->cur_url, $order, $where) ?></th>
                    <th>IP区域</th>
                    <th><?= sort_title('create_time', '注册时间', $this->cur_url, $order, $where) ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($result as $row) : ?>
                    <tr>
                        <td><?= $row['id'] ?></td>
                        <td><a href="javascript:;" onclick="user_detail(<?= $row['id'] ?>);" style="<?= $row['type'] == 1 ? 'color:#aaaaaa;' : '' ?>"><?= $row['user_name'] ?></a></td>
                        <td><?= $row['agent_name'] ?></td>
                        <td><?= $row['agent_code'] ?></td>
                        <td><?= $row['mobile'] ?></td>
                        <td><?= $row['source'] ?></td>
                        <td><?= user_model::$platformList[$row['platform']] ?></td>
                        <td style="word-break: break-all;"><?= $row['create_ua'] ?></td>
                        <td><?= $row['create_domain'] ?></td>
                        <td><?= $row['create_ip'] ?></td>
                        <td><?= $row['country'] ?></td>
