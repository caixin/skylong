...
<div class="box">
    ...
    <div class="nav-tabs-custom">
        <?php $this->load->view("{$this->router->class}/_tabs", array('type'=>0)); ?>
                            ...
                            <?php foreach ($result as $row) : ?>
                                <tr>
                                    ...
                                    <?php if ($this->session->userdata('roleid') == 1 || in_array("{$this->router->class}/edit", $this->allow_url)) : ?>
                                        <button type="button" id="status0_<?= $row['id'] ?>" class="btn <?= $row['status'] == 0 ? 'btn-info' : 'btn-default' ?>" onclick="status_row(<?= $row['id'] ?>,0)"><?= ettm_lottery_cheat_model::$status0List[0] ?></button>
                                        <button type="button" id="status1_<?= $row['id'] ?>" class="btn <?= $row['status'] == 1 ? 'btn-info' : 'btn-default' ?>" onclick="status_row(<?= $row['id'] ?>,1)"><?= ettm_lottery_cheat_model::$status0List[1] ?></button>
                                        <button type="button" id="status2_<?= $row['id'] ?>" class="btn <?= $row['status'] == 2 ? 'btn-info' : 'btn-default' ?>" onclick="status_row(<?= $row['id'] ?>,2)"><?= ettm_lottery_cheat_model::$status0List[2] ?></button>
                                    <?php else: ?>
                                        <?=ettm_lottery_cheat_model::$status0List[$row['status']]?>
                                    <?php endif; ?>
                                    </td>
                                    <td>
...
<script>
    ...
    function add() {
        layer.open({
            ...
            offset: ['20px', ''],
            area: ['50%', '90%'],
            content: '<?= site_url("{$this->router->class}/create/$where[operator_id]/0") ?>'
        });
    }
    ...
    function status_row(id, status) {
        if (status == 1) {
            ...
            $('#status2_' + id).removeClass('btn-info').addClass('btn-default');
        } else if (status == 2) {
            $('#status0_' + id).removeClass('btn-info').addClass('btn-default');
            $('#status1_' + id).removeClass('btn-info').addClass('btn-default');
            $('#status2_' + id).removeClass('btn-default').addClass('btn-info');
        } else {
