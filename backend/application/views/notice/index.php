        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th nowrap><?= sort_title('type', '公告类型', $this->cur_url, $order, $where) ?></th>
                    <th width="150"><?= sort_title('name', '公告名称', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('content', '公告内容', $this->cur_url, $order, $where) ?></th>
                    <th nowrap><?= sort_title('sort', '排序', $this->cur_url, $order, $where) ?></th>
            ...
            <tbody>
                <?php foreach ($result as $row) : ?>
                    <tr>
                        <td><?= $row['id'] ?></td>
                        <td nowrap><?= $operator[$row['operator_id']] ?></td>
                        <td nowrap><?= notice_model::$typeList[$row['type']] ?></td>
                        <td><?= $row['name'] ?></td>
                        <td><?= $row['content'] ?></td>
                        <td><?= $row['sort'] ?></td>
                        <td nowrap>
                            <?php if ($this->session->userdata('roleid') == 1 || in_array("{$this->router->class}/edit", $this->allow_url)) : ?>
                                <button type="button" id="status1_<?= $row['id'] ?>" class="btn <?= $row['status'] == 1 ? 'btn-info' : 'btn-default' ?>" onclick="status_row(<?= $row['id'] ?>,1)"><?= notice_model::$statusList[1] ?></button>
                                <button type="button" id="status0_<?= $row['id'] ?>" class="btn <?= $row['status'] == 0 ? '' : 'btn-default' ?>" onclick="status_row(<?= $row['id'] ?>,0)"><?= notice_model::$statusList[0] ?></button>
                            <?php else : ?>
                                <?= notice_model::$statusList[$row['status']] ?>
                            <?php endif; ?>
                        </td>
                        <td><?= $row['update_time'] ?></td>
                        <td nowrap><?= $row['update_by'] ?></td>
                        <td>
                            <?php if ($this->session->userdata('roleid') == 1 || in_array("{$this->router->class}/edit", $this->allow_url)) : ?>
