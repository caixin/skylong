        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th><?= sort_title('values', '投注值', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('qishu', '期数', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('bet_money', '下注金额', $this->cur_url, $order, $where) ?></th>
                    <th><?= sort_title('bet_money_max', '投注总额', $this->cur_url, $order, $where) ?></th>
...
<script>
    ...
    function edit(id) {
        layer.open({
            ...
            offset: ['20px', ''],
            area: ['50%', '90%'],
            content: '<?= site_url("{$this->router->class}/bet_edit") ?>/' + id
        });
    }
</script>